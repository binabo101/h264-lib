(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.MP4Conv = factory(global.stream));
  ...
  */
  ...
  var MP4Conv = /*#__PURE__*/function (_Event) {
    _inherits(MP4Conv, _Event);

    var _super = _createSuper(MP4Conv);

    function MP4Conv(options) {
      var _this;
    ...
    _createClass(MP4Conv, [{
      ...
      value: function createBuffer() {
        ...
        for (var type in this.remuxController.tracks) {
          ...
          if (!MP4Conv.isSupported("".concat(type, "/mp4; codecs=\"").concat(track.mp4track.codec, "\""))) {
      ...
      value: function onBuffer(data) {
        if (this.options.readFpsFromTrack && typeof data.fps !== 'undefined' && this.options.fps != data.fps) {
          ...
          log("MP4Conv changed FPS to ".concat(data.fps, " from track data"));
        }
      ...
      value: function onBufferError(data) {
        if (data.name == 'QuotaExceeded') {
          log("MP4Conv cleaning ".concat(data.type, " buffer due to QuotaExceeded error"));
          this.bufferControllers[data.type].initCleanup(this.node.currentTime);
          return;
        } else if (data.name == 'InvalidStateError') {
          log('MP4Conv is reseting due to InvalidStateError');
          this.reset();
        } else {
          this.endMSE();
        }
